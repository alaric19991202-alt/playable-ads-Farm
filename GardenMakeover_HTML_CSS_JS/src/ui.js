...
export function buildUI({
  ...
  function renderCategoryTabs() {
    ...
    CATEGORIES.forEach((c) => {
      ...
      b.className = "categoryTab" + (c.id === selectedCategory ? " selected" : "");
      b.textContent = c.emoji || c.label[0];
  ...
  function renderItems() {
    ...
    const list = ITEMS[selectedCategory] || [];

    list.forEach((it) => {
      const b = document.createElement("button");
      b.className = "itemCard" + (it.id === selectedItemId ? " selected" : "");
      ...
      icon.className = "pillEmoji";
      icon.textContent = it.currency === "diamond" ? "💎" : "🪙";
      const val = document.createElement("span");
      val.textContent = String(it.cost ?? 0);
  ...
  function showResult({ stars = 4.6, coins = 250, diamonds = 1 }) {
    ...
    const s = "★".repeat(fullStars) + (half ? "½" : "") + "☆".repeat(Math.max(0, 5 - fullStars - (half ? 1 : 0)));
    if (starText) starText.textContent = s.replace("½", "☆"); // keep classic star bar
