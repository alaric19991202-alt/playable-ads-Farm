...
export function createTemplateLibrary({ items, gltfCache, setLoading }) {
  ...
  async function load() {
    ...
    let done = 0;
    const tasks = entries.map((it) => async () => {
      let tpl = null;
      if (it.procedural) {
        tpl = buildProcedural(it.procedural);
        scaleTemplateToTarget(tpl, 1.0, it.scale ?? 1);
      } else if (it.model) {
        const gltf = await gltfCache.load(it.model);
        if (gltf.animations?.length) animations.set(it.id, gltf.animations);
        const clone = cloneSkeleton(gltf.scene);
        tpl = normalizePivotToGround(clone);
        setShadows(tpl, true);
        scaleTemplateToTarget(tpl, 1.0, it.scale ?? 1);
      }

      if (tpl) templates.set(it.id, tpl);
      done += 1;
      setLoading?.(0.22 + 0.65 * (done / total), `Preparing ${it.label}...`);
    });
