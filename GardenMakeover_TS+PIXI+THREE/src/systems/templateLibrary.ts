...
type TemplateLibraryParams = {
  items: Record<string, TemplateItem[]>;
  gltfCache: { load: (path: string) => Promise<{ scene: THREE.Object3D; animations?: THREE.AnimationClip[] }> };
...
async function runWithConcurrency(tasks: Array<() => Promise<void>>, maxConcurrent: number) {
...
function buildProcedural(kind: string) {
...
export function createTemplateLibrary({ items, gltfCache, setLoading }: TemplateLibraryParams) {
  const templates = new Map<string, THREE.Object3D>();
  const animations = new Map<string, THREE.AnimationClip[]>();

  async function load() {
    const groups = Object.entries(items);
    const entries: TemplateItem[] = [];
