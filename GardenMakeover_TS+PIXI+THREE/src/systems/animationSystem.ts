...
export function createAnimationSystem() {
  const clipsByItem = new Map<string, THREE.AnimationClip[]>();
  const activeMixers: Array<{ root: THREE.Object3D; mixer: THREE.AnimationMixer }> = [];
  const mixerMap = new WeakMap<THREE.Object3D, THREE.AnimationMixer>();
  const clock = new THREE.Clock();
  const maxDelta = 0.05;

  function registerClips(itemId: string, clips: THREE.AnimationClip[]) {
    if (!clips?.length) return;
    clipsByItem.set(itemId, clips);
  }

  function registerAll(map: Map<string, THREE.AnimationClip[]> | null | undefined) {
    if (!map) return;
  ...
  function start(root: THREE.Object3D) {
    if (!root || mixerMap.has(root)) return;
    const itemId = (root as THREE.Object3D & { userData?: { itemId?: string } }).userData?.itemId;
    if (!itemId) return;
  ...
  function stop(root: THREE.Object3D) {
