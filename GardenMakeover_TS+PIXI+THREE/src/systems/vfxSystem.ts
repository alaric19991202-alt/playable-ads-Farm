...
type VfxParams = {
  ...
  tweenNumber: (params: { from: number; to: number; duration?: number; onUpdate?: (value: number) => void; onComplete?: () => void }) => void;
};

export function createVfxSystem({ scene, smokeTexturePath, tweenNumber }: VfxParams) {
  ...
  async function load() {
    ...
    try {
      ...
      console.warn("Smoke texture load failed", err);
      smokeTex = null;
    }
  }
  ...
  function releaseSmokeSprite(sprite: THREE.Sprite | null) {
    if (!sprite) return;
    sprite.visible = false;
  ...
  function puffSmoke(pos: THREE.Vector3) {
    if (!smokeTex) return;
  ...
  function spawnPop(obj: THREE.Object3D) {
